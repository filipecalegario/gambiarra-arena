      ...
      <div className="space-y-6">
        {!story ? <div className="bg-slate-900 border border-slate-800 rounded-3xl p-7 space-y-5">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            {(story.status === 'generating' || story.status === 'voting') && <button disabled={busy} onClick={() => { if (confirm('Cancelar este capítulo? As continuações e votos dele serão descartados e a história volta para o capítulo anterior.')) request(`/story/${story.id}/chapters/cancel`); }} className="px-6 py-3 bg-rose-500/90 text-white rounded-xl font-bold">Cancelar capítulo</button>}
            {story.status === 'completed' && <p className="text-emerald-300 text-xl font-bold">História concluída — pronta para leitura!</p>}
          </div>
