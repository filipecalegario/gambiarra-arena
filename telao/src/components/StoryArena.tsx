...
export function StoryArena() {
  ...
  const candidates = story.status==='generating' ? [...new Set([...responses.map(r=>r.participantId),...Object.keys(live)])].map(id=>({id,nickname:responses.find(r=>r.participantId===id)?.nickname||'Modelo escrevendo',content:live[id]||responses.find(r=>r.participantId===id)?.content||''})) : responses.map(r=>({id:r.participantId,nickname:r.nickname,content:r.content}));
